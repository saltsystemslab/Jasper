...
# gt
gt_indices, gt_distances =  jasper.read_groundtruth("/projects/SaltSystemsLab/ann_gt//GT_10M/bigann-10M", 10)

# warmup
indices, distances = g.search(queries, k=10, beam_width=16, limit=128, print_throughput=False)

indices, distances = g.search(queries, k=10, beam_width=16, limit=128, print_throughput=True)
jasper.get_recall(gt_indices, indices, 10, len(queries))
